...
`.mxl`) files and produces a `.feedpak` with notation format data
(`notation_<instrument>.json` + `song_timeline.json`). Piano audio via
FluidSynth is generated by default but optional — see "Include
synthesized piano audio" in the UI and the Dependencies section below.
...
```
.xml / .musicxml / .mxl file
  → mxml2notation.parse_musicxml()
      → .mxl detected by zip magic number (not extension), unwrapped via
        META-INF/container.xml's primary rootfile (or the first top-level
        .xml entry as a fallback) before parsing proceeds as plain MusicXML
...
| **.mxl alternate renditions ignored** | Only the primary `<rootfile>` in `META-INF/container.xml` is read; additional renditions (e.g. an embedded PDF) are ignored. |
...
- FluidSynth binary + `GeneralUser-GS.sf2` — **audio only**, available in
  the FeedBack Docker/OrbStack container environment via core
  `gp2midi.render_midi_to_audio`; not required for notation-only builds
  (checkbox off, or `gp2midi` unavailable — either way the build still
